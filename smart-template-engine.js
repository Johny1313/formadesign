(function(root){
  ...
  function sourceLabel(links=[]){
    ...
    return names.length?`Fonte: ${names.slice(0,3).join(' · ')}`:'Origem verificada';
  }
  ...
  function buildContentModel(project={}){
    ...
    return {version:1,contract:'ronda-content-model-v1',title:String(project.title||'Carrossel FORMA'),editoria:String(project.editoria||''),source,images:library,slides};
  }
  ...
  function preflightTemplate(template,contentModel){
    ...
    if(!modelSlides.length)errors.push({code:'no-content',message:'Conteúdo importado não possui slides.'});
  ...
  function defaultTemplate(contentModel){
    ...
    const board={id:'fallback_layout',name:'Layout seguro FORMA',width:w,height:h,w,h,bg:'#f3f1ed',bgMode:'solid',gradient:null,templateRole:'content',elements,semanticSlots:['IMAGE','ROLE','TITLE','BODY','SOURCE']};
    return {templateVersion:2,engineVersion:VERSION,id:'ronda-safe-default',name:'Layout seguro FORMA',category:'Sistema',smart:true,slotCount:5,project:{version:5,artboards:[board],activeArtboardId:board.id,docTitle:contentModel?.title||'FORMA'}};
  }
