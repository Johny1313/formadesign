...
export async function handleFormaAiApi(request, env) {
  const url = new URL(request.url);

  if (url.pathname === '/api/health') {
    return json({ ok: true, service: 'forma-design', version: '0.7.7', engineVersion: ENGINE_VERSION, build: BUILD_ID, runtime: 'cloudflare-workers' });
  }
  ...
  if ((url.pathname === '/api/ai/status' || url.pathname === '/api/ai/engines') && request.method === 'GET') {
    ...
    return json({
      ok: true,
      engine: 'FORMA Multi Image Engine',
...
export default {
  async fetch(request, env) {
    const url = new URL(request.url);
    if (url.pathname.startsWith('/api/')) return handleFormaAiApi(request, env);
