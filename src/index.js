...
async function runFlux1(env, prompt, quality, seed) {
...
async function generateOne(env, input, index) {
  ...
  const seed = Number.isFinite(baseSeed) ? Math.abs(Math.trunc(baseSeed + index)) : Math.floor(Math.random() * 2147483646) + 1;

  if (requested === 'flux1-schnell') {
    try {
      return await runFlux1(env, prompt, input.quality, seed);
    } catch (firstError) {
      const fallback = await runFlux2(env, prompt, width, height, seed);
      return { ...fallback, fallbackFrom: firstError.message };
    }
  }

  try {
    return await runFlux2(env, prompt, width, height, seed);
  } catch (firstError) {
    const fallback = await runFlux1(env, prompt, input.quality, seed);
    return { ...fallback, fallbackFrom: firstError.message };
  }
}
...
async function handleApi(request, env) {
  ...
  if (url.pathname === '/api/health') {
    return json({ ok: true, service: 'forma-design', version: '0.6.0', runtime: 'cloudflare-workers' });
  }
  ...
  if (url.pathname === '/api/ai/status' && request.method === 'GET') {
    ...
    return json({
      ...
      capabilities: ['text-to-image', 'article-analysis', 'provider-fallback'],
    });
  }
