...
const checks = [
  ...
  ['Text rasterizer builds foreignObject SVG snapshots', /function buildTextForeignObjectSvg\(content,width,height\)/],
  ['Text rasterizer serializes DOM to SVG', /new XMLSerializer\(\)\.serializeToString\(wrapper\)/],
  ['Static board export pre-rasterizes text', /if\(e\.type==='text'\)\{assets\.set\(e\.id,await rasterizeTextElementFromDom\(e\)\);return;\}/],
  ['Video assets pre-rasterize text', /if\(e\.type==='text'\)\{assets\.set\(e\.id,await rasterizeTextElementFromDom\(e\)\);return;\}/],
