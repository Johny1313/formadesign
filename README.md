...
## Chart responsive resize — v0.9.7.5.49
...
## v0.9.7.5.49 — Layout, palette themes, fill/stroke and export chooser
...
## Shared library + Projects — v0.9.7.5.49
...
## Clipboard e agrupamento — v0.9.7.5.49
...
## Rich text + alinhamento — v0.9.7.5.49
...
## Clipboard + planilha de colagem — v0.9.7.5.49
...
## Planilha — cabeçalho obrigatório na primeira linha (v0.9.7.5.49)
...
## Video fidelity lock — v0.9.7.5.49
...
## Board video fidelity — v0.9.7.5.49
...
## Hub editorial de imagens — atualização escalonada 0.9.7.5.41 → 0.9.7.5.49

### 0.9.7.5.41 — Foto Agência
- Conector direto para a página pública Foto Agência / Agência Brasil.
- Busca por assunto com crédito, link de origem e status de revisão editorial.
- O FORMA não contorna login, paywall ou restrições de download.

### 0.9.7.5.42 — Fotos Públicas
- Busca editorial em `fotospublicas.com` integrada ao mesmo painel.
- Crédito, tipo de uso e link da publicação preservados.
- Conteúdo marcado para conferência antes da publicação.

### 0.9.7.5.43 — Bancos gratuitos ampliados
- Openverse por API pública anônima.
- Wikimedia Commons preservado.
- Pexels opcional via secret `PEXELS_API_KEY`.
- Unsplash opcional via secret `UNSPLASH_ACCESS_KEY`, incluindo chamada de download tracking exigida pela API.

### 0.9.7.5.44 — Ranking editorial e segurança
- Busca paralela com `Promise.allSettled`.
- Agência Brasil e Fotos Públicas recebem prioridade em contexto factual brasileiro.
- Licenças abertas e stock entram depois.
- Proxy de imagem mantém allowlist de hosts para reduzir risco de SSRF.
- Cada resultado mantém fonte, crédito, licença/status de uso e link original.

### 0.9.7.5.49 — Fluxo final factual → banco → IA
- UX única de busca com filtro por fonte.
- IA passa a ser fallback explícito.
- Prompt de IA enriquecido por estilo editorial/documental/conceitual/ilustração.
- Proporção da prancheta orienta a geração em paisagem, retrato ou quadrado.
- Toda a bateria de regressão anterior permanece ativa.
...
Sem esses secrets, Pexels e Unsplash não são acionados. Agência Brasil, Fotos Públicas, Openverse e Wikimedia continuam disponíveis sem esses secrets.

## Máscara por camada + Caneta — v0.9.7.5.49

### Máscara por camada superior
- Qualquer elemento pode habilitar `Usar a camada imediatamente acima como máscara`.
- A camada imediatamente superior vira a fonte da máscara e é identificada na lista de Camadas como `MÁSCARA`.
- Modos disponíveis: `Mostrar o que está dentro` e `Ocultar o que está dentro` (inverso).
- O objeto usado como máscara continua editável pela lista de camadas; no canvas aparece como guia translúcida quando ele ou o conteúdo mascarado está selecionado.
- A máscara é não destrutiva e persiste em projetos, duplicação, clipboard e snapshots.
- Exportações estáticas e de vídeo usam composição real por alpha (`destination-in` / `destination-out`) e não apenas um efeito CSS de preview.
- Se a camada superior for removida/reordenada, o editor mantém a configuração e informa que não há uma fonte de máscara disponível.

### Caneta
- Novo botão `Caneta` na aba Formas.
- Clique na prancheta para criar pontos; `Enter` ou duplo clique conclui; `Esc` cancela.
- Dois pontos criam um caminho aberto. Três ou mais pontos criam uma forma fechada.
- A forma criada usa o mesmo painel de preenchimento/stroke das demais formas.
...
- texto colado é processado diretamente;
- Evidence Pack é criado e persistido antes da geração;
- Workers AI é usado para o Carousel AI quando o binding estiver disponível;
- se a IA estiver indisponível ou retornar estrutura inválida, entra `deterministic-fallback` baseado somente na Evidence;
- Factual Gate verifica Evidence IDs e números suportados pela fonte;
- Quality Gate valida quantidade, duplicação, limites e suporte factual;
- o resultado `ready` pode ser aberto diretamente no Canvas do FORMA;
